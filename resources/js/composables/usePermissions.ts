...
export function usePermission() {
   ...
   =========================== */
    ...
    const can = (path: string) => {
        return path.split('.').reduce((acc: any, key) => acc?.[key], permissions) === true;
    };

    const canAny = (paths: string[]) => paths.some((p) => can(p));

    const canAll = (paths: string[]) => paths.every((p) => can(p));
