...
export function useStudentForm(form: any, validateAllFields: () => boolean, validationErrors: any, showValidation: any, studentFormRef?: Ref<any>) {
            ...
            form.transform((data: any) => {
                ...
                for (const [key, value] of Object.entries(data)) {
                    if (value !== null && value !== '' && value !== undefined) {
                        if (['stateId', 'districtId', 'municipalityId', 'classId', 'sectionId'].includes(key)) {
                            transformedData[key] =
                                typeof value === 'object' && !(value instanceof File) && value !== null && 'value' in value
                ...
                onError: (errors: any) => {
                    ...
                    toast.error('Failed to register student. Please check the form and try again.', {
                        duration: 5000,
                        position: 'top-right',
                        description: Object.keys(errors).length > 0 ? `Found ${Object.keys(errors).length} error(s)` : 'An unexpected error occurred',
                        action: {
                            label: 'Close',
                            onClick: (e) => {
                                // Close action
