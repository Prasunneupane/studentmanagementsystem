...
// Composables
const { students, loading, loadByDateRange, updateStudent, deleteStudent, getGuardians, createGuardian, updateGuardian, deleteGuardian } =
    useStudents();
...
const startEdit = async (student: Student) => {
    ...
    try {
        ...
        console.log(municipalities.value, 'municipalities.value');
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex items-center gap-2' }, [
                can('students.canView') &&
                    h(
                        Button,
                        { variant: 'ghost', size: 'sm', class: 'h-8 w-8 p-0 cursor-pointer', title: 'View', onClick: () => handleView(student) },
                        () => h(Eye, { class: 'h-4 w-4' }),
                    ),
                can('students.canEdit') &&
                    h(
                        Button,
                        { variant: 'ghost', size: 'sm', class: 'h-8 w-8 p-0 cursor-pointer', title: 'Edit', onClick: () => handleEdit(student) },
...
<template>
                ...
                <CardContent>
                    <div class="mb-6 flex flex-col items-end gap-4 sm:flex-row">
                        <div class="flex-1 space-y-2 sm:max-w-[250px]">
                            ...
                            <DatePicker :model-value="fromDateValue" month-year-selector placeholder="From" />
                        </div>
                        <div class="flex-1 space-y-2 sm:max-w-[250px]">
                            <Label>To Date</Label>
                            <DatePicker :model-value="toDateValue" month-year-selector placeholder="To" />
                        </div>
    ...
    <Dialog v-model:open="isDialogOpen">
        <DialogContent class="flex w-full max-w-[95vw] flex-col overflow-hidden p-0 lg:max-w-[1100px]" style="height: 85vh">
                ...
                <div class="flex-1 overflow-y-auto p-6">
                            ...
                            <div>
                                ...
                                <dl class="space-y-2 text-sm">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <dd class="lowercase">{{ selectedStudent?.email ?? '—' }}</dd>
                                    </div>
                                    <div class="flex justify-between">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <dd>{{ selectedStudent?.age ?? '—' }} yrs</dd>
                                    </div>
                                    <div class="flex justify-between">
                                        <dt class="text-gray-600">DOB</dt>
                                        <dd>{{ selectedStudent?.date_of_birth ?? '—' }}</dd>
                                    </div>
                                    <div class="flex justify-between">
                                        <dt class="text-gray-600">Joined</dt>
                                        <dd>{{ selectedStudent?.joined_date ?? '—' }}</dd>
                                    </div>
                                </dl>
                            </div>
                            <div>
                    ...
                    <TabsContent value="guardians" class="mt-0">
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            <Button size="sm" @click="openGuardianModal()"> <Plus class="mr-1 h-4 w-4" /> Add Guardian </Button>
                        </div>
                            ...
                            <table class="w-full min-w-[800px] text-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr v-else-if="guardians.length === 0">
                                        <td colspan="8" class="py-10 text-center text-gray-500">No guardians added</td>
                                    </tr>
                                    <tr v-for="g in guardians" :key="g.id" class="border-t hover:bg-gray-50">
                                        <td class="max-w-[200px] truncate px-4 py-3" :title="g.name">{{ g.name || '—' }}</td>
                                        <td class="max-w-[200px] truncate px-4 py-3" :title="g.relation">{{ g.relation || '—' }}</td>
                                        <td class="max-w-[200px] truncate px-4 py-3" :title="g.phone">{{ g.phone || '—' }}</td>
                                        <td class="max-w-[200px] truncate px-4 py-3" :title="g.email">{{ g.email || '—' }}</td>
                                        <td class="max-w-[200px] truncate px-4 py-3" :title="g.occupation">{{ g.occupation || '—' }}</td>
                                        <td class="max-w-[200px] truncate px-4 py-3" :title="g.address">{{ g.address || '—' }}</td>
                                        <td class="px-4 py-3">
                                            <span
                                                v-if="g.is_primary_contact"
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex items-center justify-end gap-1">
                                                <Button
                                                    v-if="can('guardians.canEdit')"
                                                    variant="ghost"
                                                    size="icon"
                                                    class="h-8 w-8"
                                                    @click="openGuardianModal(g)"
                                                >
                                                    <Pencil class="h-4 w-4" />
                                                </Button>
                                                <Button
                                                    v-if="can('guardians.canDelete')"
                                                    variant="ghost"
                                                    size="icon"
                                                    class="h-8 w-8 text-red-600 hover:bg-red-50 hover:text-red-700"
                                                    @click="handleGuardianDelete(g)"
                                                >
            ...
            <div class="flex flex-shrink-0 justify-end gap-3 border-t bg-white p-4">
            ...
            <div class="max-h-[calc(85vh-140px)] min-h-0 flex-1 overflow-y-auto p-6">
                ...
                <form v-else @submit.prevent="submitEditForm" class="space-y-4">
                    <div class="flex items-center gap-4 rounded-lg bg-gray-50 p-4">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-xs text-gray-500">Upload a new photo or keep existing one</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="space-y-2">
                            ...
                            <CustomSelect v-model="editForm.classId" :options="classes" placeholder="Select Class" />
                        ...
                        <div class="space-y-2">
                            ...
                            <CustomSelect id="edit-sectionId" v-model="editForm.sectionId" :options="sections" placeholder="Select Section" />
                        </div>
              ...
              <p v-if="editValidationErrors.joinedDate" class="text-sm text-red-600">{{ editValidationErrors.joinedDate }}</p>
                        ...
                        <div class="space-y-2">
                            ...
                            <CustomSelect id="edit-stateId" v-model="editForm.stateId" :options="states" placeholder="Select State" />
        ...
        <DialogContent class="max-w-4xl">
            ...
            <form v-if="editingGuardian" @submit.prevent="submitGuardian" class="space-y-6">
                <div>
                    ...
                    <p v-if="guardianErrors.name" class="mt-1 text-sm text-red-600">{{ guardianErrors.name }}</p>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Label class="cursor-pointer font-normal">Primary Contact</Label>
                </div>
                <DialogFooter>
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    ...
                    <div class="mt-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm">
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    This action cannot be undone.
                    <div class="mt-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm">
                        <strong>{{ editingGuardian?.name }}</strong
                        ><br />
