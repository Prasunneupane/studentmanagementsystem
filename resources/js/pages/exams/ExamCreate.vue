...
import { BookOpen, CheckCircle2, ChevronLeft, ChevronRight, Loader2, Save, Users, X } from 'lucide-vue-next';
...
const toggleSection = (classId: string, sectionId: string) => {
    ...
    if (sel.all) {
        ...
        cls.sections.forEach((s) => {
            if (s.id !== sectionId) sel.sections.add(s.id);
        });
    } else {
...
const allClassesSelected = computed(() => props.classes.length > 0 && props.classes.every((cls) => classSelections.value[cls.id]?.all));
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="flex items-center gap-0">
                <template v-for="(step, i) in steps" :key="step.number">
                    <div class="flex items-center gap-2">
                        <div
                            ...
                            :class="{
                                ...
                                'border-green-500 bg-green-500 text-white': currentStep > step.number,
                                'border-border bg-muted text-muted-foreground': currentStep < step.number,
                            }"
                        >
                            <CheckCircle2 v-if="currentStep > step.number" class="h-5 w-5" />
                    ...
                    <div
                        ...
                        class="mx-3 h-0.5 flex-1 transition-all duration-300"
            ...
            <!-- STEP 1: EXAM DETAILS                      -->
            <!-- ══════════════════════════════════════════ -->
            <Card v-if="currentStep === 1" class="w-full rounded-2xl shadow-lg">
                <CardHeader class="border-b">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-sm text-muted-foreground">Enter basic exam information</p>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardContent class="pt-6">
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div class="space-y-2">
                                <Label for="term_id">Term <span class="text-xs text-muted-foreground">(optional)</span></Label>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                            <div class="space-y-2">
                                ...
                                <DatePicker
                                    ...
                                    @update:model-value="
                                        (val) => {
                                            form.start_date = formatDate(val);
                                        }
                                    "
                                />
                            ...
                            <div class="space-y-2">
                                <Label for="weightage">Weightage (%) <span class="text-xs text-muted-foreground">(optional)</span></Label>
                        ...
                        <div class="rounded-lg bg-muted/50 p-4">
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="grid gap-1 leading-none">
                                    <Label for="is_published" class="cursor-pointer font-medium">Publish Immediately</Label>
            ...
            <!-- STEP 2: CLASS & SECTION SELECTION         -->
            <!-- ══════════════════════════════════════════ -->
            <Card v-if="currentStep === 2" class="w-full rounded-2xl shadow-lg">
                <CardHeader class="border-b">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-sm text-muted-foreground">Choose which classes this exam applies to</p>
                            </div>
                        </div>
                        <!-- Selection count badge -->
                        <div v-if="totalSelectedCount > 0" class="rounded-full bg-primary/10 px-3 py-1.5 text-sm font-semibold text-primary">
                ...
                <CardContent class="pt-6">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-3 rounded-lg border bg-muted/50 p-3">
                            ...
                            />
                            <Label for="select_all" class="cursor-pointer text-sm font-semibold" @click="selectAllClasses">
                        ...
                        <div class="space-y-3">
                            <div
                                ...
                                :class="
                                    classSelections[cls.id]?.all || classSelections[cls.id]?.sections.size > 0
                            ...
                            >
                                <!-- Class header row -->
                                <div
                                    class="flex cursor-pointer items-center gap-3 p-3 transition-colors hover:bg-muted/30"
                                ...
                                >
                                    <Checkbox
                                        ...
                                        @click.stop
                                    />
                                    <Label :for="`class_${cls.id}`" class="pointer-events-none flex-1 cursor-pointer font-semibold">
                                        {{ cls.name }}
                                    </Label>
                                ...
                                <div class="flex flex-wrap gap-2 px-10 pb-3">
                                    <div
                                        ...
                                        :key="section.id"
                                        class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-1.5 text-sm transition-all duration-150 select-none"
                                        :class="
                                            isSectionChecked(cls.id, section.id)
                        ...
                        <!-- Validation message -->
                        <p v-if="!step2Valid && totalSelectedCount === 0" class="flex items-center gap-1.5 text-sm text-amber-600">
                        ...
                        <div class="flex justify-between gap-3 border-t pt-4">
                            <Button type="button" variant="outline" @click="goBack"> <ChevronLeft class="mr-2 h-4 w-4" /> Back </Button>
                            <div class="flex gap-3">
                                <Button type="button" variant="outline" @click="router.visit('/exams')"> <X class="mr-2 h-4 w-4" /> Cancel </Button>
