...
import { ArrowLeft, BookOpen, Calendar, CheckCircle2, FileDown, FileSpreadsheet, FileText, Pencil, Plus, Printer } from 'lucide-vue-next';
...
const activeTab = ref<string>(props.groupedSchedule.length ? `${props.groupedSchedule[0].class_id}_${props.groupedSchedule[0].section_id}` : '');

const activeGroup = computed(() => props.groupedSchedule.find((g) => `${g.class_id}_${g.section_id}` === activeTab.value) ?? null);
...
// ── Stats ─────────────────────────────────────────────────────────────
const totalSchedules = computed(() => props.groupedSchedule.reduce((acc, g) => acc + g.schedules.length, 0));
...
const examTypeLabel = (type: string) => type?.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()) ?? '';
...
const getSubjectColor = (name: string) => {
    if (!subjectColorMap[name]) subjectColorMap[name] = subjectColors[colorIndex++ % subjectColors.length];
...
const handleExcel = async () => {
    ...
    try {
        ...
        wsSchedule['!cols'] = [{ wch: 5 }, { wch: 14 }, { wch: 12 }, { wch: 22 }, { wch: 13 }, { wch: 11 }, { wch: 11 }, { wch: 10 }];
        ...
        props.groupedSchedule.forEach((grp) => {
            ...
            const wsClass = XLSX.utils.aoa_to_sheet([['#', 'Subject', 'Date', 'Start Time', 'End Time', 'Room No'], ...classRows]);
            wsClass['!cols'] = [{ wch: 4 }, { wch: 22 }, { wch: 13 }, { wch: 11 }, { wch: 11 }, { wch: 10 }];
        ...
        <div class="mx-auto flex w-full max-w-7xl flex-col gap-4 p-4">
                ...
                <CardContent class="p-5">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <Button variant="outline" size="sm" @click="router.visit('/exams')"> <ArrowLeft class="mr-1 h-4 w-4" /> Back </Button>
                            <Button
                                variant="outline"
                                size="sm"
                                @click="router.visit(`/exams/${exam.id}/schedule/edit`)"
                                class="border-amber-300 text-amber-700 hover:bg-amber-50"
                            >
                                <Pencil class="mr-1.5 h-4 w-4" />
                                Edit Schedule
                            </Button>
                            <Button variant="outline" size="sm" @click="handlePrint" class="border-slate-300 hover:bg-slate-50">
                                <Printer class="mr-1.5 h-4 w-4 text-slate-600" />
                                Print All
                            </Button>
                            <Button
                                variant="outline"
                            ...
                            <Button size="sm" @click="router.visit(`/exams/${exam.id}/schedule`)">
                                <Plus class="mr-1 h-4 w-4" /> Add Schedule
                            </Button>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="rounded-xl border bg-card p-4">
                    ...
                    <div class="mt-0.5 text-xs font-medium tracking-wide text-muted-foreground uppercase">Classes</div>
                </div>
                <div class="rounded-xl border bg-card p-4">
                    <div class="text-2xl font-bold tracking-tight">{{ totalSections }}</div>
                    <div class="mt-0.5 text-xs font-medium tracking-wide text-muted-foreground uppercase">Sections</div>
                </div>
                <div class="rounded-xl border bg-card p-4">
                    <div class="text-2xl font-bold tracking-tight">{{ totalSchedules }}</div>
                    <div class="mt-0.5 text-xs font-medium tracking-wide text-muted-foreground uppercase">Schedules</div>
                </div>
            </div>

            <!-- ── Empty state ─────────────────────────────────────────────── -->
            <div v-if="groupedSchedule.length === 0" class="flex flex-col items-center justify-center rounded-2xl border bg-card py-16 text-center">
                <BookOpen class="mb-3 h-10 w-10 text-muted-foreground/40" />
                <p class="text-base font-medium text-muted-foreground">No schedules found</p>
                <p class="mt-1 text-sm text-muted-foreground/70">Add a schedule to get started</p>
                <Button class="mt-4" size="sm" @click="router.visit(`/exams/${exam.id}/schedule`)">
                    <Plus class="mr-1.5 h-4 w-4" /> Add Schedule
                </Button>
            </div>

            <!-- ── Tab layout ──────────────────────────────────────────────── -->
            <div v-else class="flex min-h-[400px] gap-4">
                <!-- Left sidebar: Class → Section tabs, sorted Class 1, Class 2 … -->
                <div class="w-52 shrink-0 overflow-hidden rounded-2xl border bg-card py-3">
                    <div v-for="cls in classTabs" :key="cls.class_id" class="mb-3 last:mb-0">
                        <!-- Class header -->
                        <p class="px-4 pb-1.5 text-xs font-bold tracking-wider text-muted-foreground uppercase">
                            {{ cls.class_name }}
                        </p>
                        <!-- Section buttons -->
                        <button
                            v-for="grp in cls.sections"
                            ...
                            :class="
                                activeTab === `${grp.class_id}_${grp.section_id}`
                ...
                <div class="flex-1 overflow-hidden rounded-2xl border bg-card">
                    <div v-if="activeGroup">
                        <!-- Section header bar -->
                        <div class="flex items-center justify-between border-b bg-muted/20 px-5 py-3.5">
                            <div class="flex items-center gap-2">
                                <div class="h-2 w-2 rounded-full bg-primary"></div>
                                <span class="text-sm font-semibold">{{ activeGroup.class_name }}</span>
                            ...
                            <Button variant="outline" size="sm" @click="handlePrintSection" class="h-7 border-slate-300 text-xs hover:bg-slate-50">
                                <Printer class="mr-1.5 h-3.5 w-3.5 text-slate-600" />
                                Print this section
                            </Button>
                        </div>
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b bg-muted/10">
                                        <th class="px-4 py-2.5 text-left text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                                            Subject
                                        </th>
                                        <th class="px-4 py-2.5 text-left text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                                            Date
                                        ...
                                        <th class="hidden px-4 py-2.5 text-left text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                                            Theory
                                        </th>
                                        <th class="hidden px-4 py-2.5 text-left text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                                            Practical
                                ...
                                <tbody>
                                    <tr
                                        v-for="row in activeGroup.schedules"
                                        :key="row.id"
                                    ...
                                    >
                                        ...
                                        <td class="hidden px-4 py-3">
                                            <span class="rounded-md bg-blue-50 px-2 py-1 font-mono text-xs font-semibold text-blue-700">
                                                {{ row.max_theory_marks ?? '—' }}
                                            </span>
                                        </td>
                                        <td class="hidden px-4 py-3">
                                            <span
                                                class="rounded-md px-2 py-1 font-mono text-xs font-semibold"
                                        ...
                                        <td class="hidden px-4 py-3">
                                            <span class="rounded-md bg-purple-50 px-2 py-1 font-mono text-xs font-semibold text-purple-700">
                                                {{ row.max_total_marks }}
                                            </span>
                                        </td>
                                        <td class="hidden px-4 py-3">
                                            <span class="rounded-md bg-amber-50 px-2 py-1 font-mono text-xs font-semibold text-amber-700">
                                                {{ row.pass_marks }}
                                            </span>
                                        </td>
                                    </tr>
                    ...
                    <div v-else class="flex h-full items-center justify-center py-20 text-muted-foreground">
