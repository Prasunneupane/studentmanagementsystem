...
const getStudentName = (student: ResultRow['student']) => {
    ...
    return student.middle_name ? `${student.first_name} ${student.middle_name} ${student.last_name}` : `${student.first_name} ${student.last_name}`;
};
...
const tableDataForPDF = computed(() => {
    return props.results.map((s, i) => ({
        ...
        grade: s.grade,
        gpa: s.gpa,
        status: s.status === 'pass' ? 'Pass' : s.status === 'fail' ? 'Fail' : 'Withheld',
    }));
});
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div v-if="results.length > 0" class="flex gap-2">
                    ...
                    <Button
                        ...
                        :disabled="finalizing || stats.finalized"
            ...
            <!-- Stats Cards -->
            <div v-if="results.length > 0" class="grid grid-cols-2 gap-3 md:grid-cols-5">
                <div class="rounded-xl border bg-card p-4 text-center">
                    <p class="mb-1 text-xs tracking-wider text-muted-foreground uppercase">Total Students</p>
                    <p class="text-2xl font-bold">{{ stats.total }}</p>
                </div>
                <div class="rounded-xl border bg-card p-4 text-center">
                    <p class="mb-1 text-xs tracking-wider text-muted-foreground uppercase">Passed</p>
                    <p class="text-2xl font-bold text-green-600">{{ stats.passed }}</p>
                </div>
                <div class="rounded-xl border bg-card p-4 text-center">
                    <p class="mb-1 text-xs tracking-wider text-muted-foreground uppercase">Failed</p>
                    <p class="text-2xl font-bold text-red-600">{{ stats.failed }}</p>
                </div>
                <div class="rounded-xl border bg-card p-4 text-center">
                    <p class="mb-1 text-xs tracking-wider text-muted-foreground uppercase">Avg %</p>
                    <p class="text-2xl font-bold">{{ stats.avgPercentage }}%</p>
                </div>
                <div class="rounded-xl border bg-card p-4 text-center">
                    <p class="mb-1 text-xs tracking-wider text-muted-foreground uppercase">Highest</p>
                    <p class="text-2xl font-bold text-primary">{{ stats.highestMarks }}</p>
                </div>
            </div>

            <!-- Results Table -->
            <Card v-if="results.length > 0" class="rounded-2xl shadow-lg">
                <CardHeader class="border-b">
                    <div class="flex items-center gap-3">
                        ...
                        <span
                            v-if="stats.finalized"
                            class="ml-2 rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700 dark:bg-green-900/30"
                        >
                            ✓ Finalized
                        </span>
                        <span v-else class="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700 dark:bg-amber-900/30">
                            Draft
                        </span>
                    </div>
                    <div class="ml-auto">
                        <div class="flex items-center gap-2">
                            <ExportPdf
                                ...
                                :data="tableDataForPDF"
                            />
                            <ExportExcel
                                :data="tableDataForPDF"
                                :headers="headersForPDF"
                                :filename="`${examName} Results.xlsx`"
                                :company-name="companyName"
                                :title="'Exam Result'"
                            />
                            <!-- Search -->
                            <div class="relative">
                                <Search class="" />
                                <Input v-model="searchQuery" placeholder="Search name,grade,status..." class="h-8 w-52 pl-8 text-xs" />
                            </div>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardContent class="p-0 pt-0">
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b bg-muted/60">
                                    <th class="w-16 px-3 py-3 text-center font-semibold text-muted-foreground">Rank</th>
                                    <th class="w-20 px-4 py-3 text-left font-semibold text-muted-foreground">Student Name</th>
                                    <th class="w-28 px-3 py-3 text-center font-semibold text-muted-foreground">Marks</th>
                                    <!-- <th class="text-center px-3 py-3 font-semibold text-muted-foreground w-24">Percentage</th> -->
                                    <th class="w-20 px-3 py-3 text-center font-semibold text-muted-foreground">Grade</th>
                                    <th class="w-20 px-3 py-3 text-center font-semibold text-muted-foreground">GPA</th>
                                    <th class="w-24 px-3 py-3 text-center font-semibold text-muted-foreground">Status</th>
                                    <th class="w-24 px-3 py-3 text-center font-semibold text-muted-foreground">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
                    ...
                    {{ results[origIndex].percentage }}%
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        ...
                                        >
                                            <CheckCircle2 v-if="results[origIndex].status === 'pass'" class="h-3 w-3" />
                    ...
                    <div v-if="filteredIndices.length > 0" class="flex flex-wrap items-center justify-between gap-3 border-t bg-muted/20 px-4 py-3">
                        ...
                        <div class="flex items-center gap-1">
                            <!-- First -->
                            <Button variant="outline" size="icon" class="h-7 w-7" :disabled="currentPage === 1" @click="currentPage = 1">
                                <ChevronsLeft class="h-3.5 w-3.5" />
                            </Button>
                            <!-- Prev -->
                            <Button variant="outline" size="icon" class="h-7 w-7" :disabled="currentPage === 1" @click="currentPage--">
                            ...
                            <template v-if="pageWindow[0] > 1">
                                ...
                                <span v-if="pageWindow[0] > 2" class="px-1 text-xs text-muted-foreground">…</span>
                            </template>
                            ...
                            <template v-if="pageWindow[pageWindow.length - 1] < totalPages">
                                <span v-if="pageWindow[pageWindow.length - 1] < totalPages - 1" class="px-1 text-xs text-muted-foreground">…</span>
                            ...
                            <!-- Next -->
                            <Button variant="outline" size="icon" class="h-7 w-7" :disabled="currentPage === totalPages" @click="currentPage++">
            ...
            <Card v-else-if="selectedClass && selectedSection" class="rounded-2xl shadow-lg">
