...
import { Edit, Filter, Plus, Trash2, UserCheck } from 'lucide-vue-next';
...
const breadcrumbs = [{ title: 'Class Teachers', href: '/class-teachers' }];

const selectedAcademicYear = ref<Option | ''>(props.academicYears.find((y) => String(y.value) === String(props.currentAcademicYearId)) || '');
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex items-center gap-2' }, [
                ...
                isClassTeacher &&
                    h(Badge, { variant: 'default', class: 'text-xs' }, () => [h(UserCheck, { class: 'h-3 w-3 mr-1' }), 'Class Teacher']),
            ]);
        },
    },
    {
        ...
        <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <CardHeader>
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">Manage teacher assignments for classes and sections</p>
                        </div>
                ...
                <CardContent>
                    <div class="flex flex-col items-end gap-4 sm:flex-row">
                        ...
                        <div class="flex-1 space-y-2 sm:max-w-[250px]">
                            ...
                            <CustomSelect v-model="selectedClass" :options="classes" placeholder="All Classes" />
                        </div>

                        <div class="flex-1 space-y-2 sm:max-w-[250px]">
                            <Label>Section (Filter)</Label>
                            <CustomSelect v-model="selectedSection" :options="sections" :disabled="!selectedClass" placeholder="All Sections" />
                        </div>

                        <Button variant="outline" @click="clearFilters" :disabled="!selectedClass && !selectedSection">
            ...
            <Card class="w-full rounded-2xl shadow-lg">
                <CardContent class="pt-6">
                    <DataTable :columns="columns" :data="filteredAssignments" :loading="false" title="Teacher Assignments" />
                </CardContent>
            </Card>
        </div>
    </AppLayout>
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    ...
                    <div v-if="assignmentToDelete" class="mt-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm">
                        <strong>{{ assignmentToDelete.teacher_name }}</strong
                        ><br />
