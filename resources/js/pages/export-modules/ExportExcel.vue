...
const exportExcel = () => {
    ...
    try {
        ...
        */
        ...
        if (props.companyName || props.title || props.metadata.length) {
            rows.push([]);
        }
        ...
        Headers
        */
        rows.push(props.headers.map((h) => h.title));
        ...
        */
        ...
        Header Row
        */
        const headerRow = rowIndex + props.metadata.length + 2;
...
<template>
    <Button :variant="variant" :size="size" :disabled="loading" @click="exportExcel">
        ...
        <FileSpreadsheet v-else class="mr-1.5 h-4 w-4" />
        {{ label }}
    </Button>
</template>
