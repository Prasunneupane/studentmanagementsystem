...
const exportPDF = async () => {
    ...
    try {
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text(props.companyName, 14, doc.internal.pageSize.getHeight() - 8);
        }

        // ── Save ──────────────────────────────────────────────────────
        const filename = `marks_${props.exam.name}_${props.subjectName}.pdf`.replace(/\s+/g, '_').toLowerCase();
        doc.save(filename);
...
<template>
    <Button :variant="variant" :size="size" :disabled="loading" @click="exportPDF">
        ...
        <FileDown v-else class="mr-1.5 h-4 w-4" />
        {{ label }}
    </Button>
</template>
