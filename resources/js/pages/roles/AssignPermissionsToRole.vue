...
import { CheckSquare, Eye, Loader2, Plus, Square } from 'lucide-vue-next';
...
const toggleModule = (moduleName: string) => {
    ...
    if (isFullySelected) {
        ...
        selectedPermissions.value = selectedPermissions.value.filter((id) => !modulePermissions.some((p) => p.id === id));
    } else {
        // Select all permissions in this module
        const modulePermissionIds = modulePermissions.map((p) => p.id);
        selectedPermissions.value = [...selectedPermissions.value.filter((id) => !modulePermissionIds.includes(id)), ...modulePermissionIds];
    }
};
...
/* -------------------- Form -------------------- */
const form = useForm({
    role_id: props.role ? { value: String(props.role.id), label: props.role.name } : null,
});
...
<template>
    ...
    >
        ...
        <div class="container mx-auto max-w-[1400px] p-6">
            <Card>
                <!-- HEADER -->
                <CardHeader class="flex items-center justify-between">
                    <div>
                        <CardTitle>Assign Permissions to Role</CardTitle>
                        <CardDescription> Select a role and assign permissions organized by module </CardDescription>
                    </div>
                    <div class="flex gap-3">
                        ...
                        <Button v-if="can('roles.canCreate')" as-child>
                            <Link :href="route('roles.create')"> <Plus class="mr-2 h-4 w-4" /> Create Role </Link>
                        </Button>
                    </div>
                </CardHeader>
                ...
                <CardContent>
                    <form @submit.prevent="handleSubmit" class="space-y-8">
                        ...
                        <div class="max-w-md space-y-2">
                            ...
                            <SelectSearch v-model="form.role_id" :options="roleOptions" placeholder="Select role" :disabled="processing" />
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between">
                                <Label class="text-lg font-semibold"> Permissions </Label>
                            ...
                            <div v-if="loadingPermissions" class="flex items-center justify-center py-12">
                                ...
                                <span class="ml-3 text-muted-foreground"> Loading permissions... </span>
                            </div>
                            ...
                            <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                ...
                                >
                                    <!-- Module Header -->
                                    <div
                                        class="flex cursor-pointer items-center gap-2 border-b bg-muted/50 p-3 transition-colors hover:bg-muted"
                                        @click="toggleModule(moduleName)"
                                    >
                                        <div class="flex-shrink-0">
                                            <CheckSquare v-if="isModuleFullySelected(moduleName)" class="h-5 w-5 text-primary" />
                                            <div
                                                v-else-if="isModulePartiallySelected(moduleName)"
                                                class="flex h-5 w-5 items-center justify-center rounded border-2 border-primary"
                                            >
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <h3 class="truncate text-sm font-semibold">{{ moduleName }}</h3>
                                    ...
                                    <div class="max-h-64 space-y-2 overflow-y-auto p-3">
                                        <div
                                            ...
                                            :key="permission.id"
                                            class="flex items-start gap-2 rounded p-1.5 transition-colors hover:bg-muted/30"
                                        >
                                            <input
                                                type="checkbox"
                                                ...
                                                :disabled="processing"
                                                class="mt-0.5 h-4 w-4 cursor-pointer rounded border border-zinc-300 accent-black focus:ring-black/40 disabled:cursor-not-allowed disabled:opacity-50"
                                            />
                                            <Label :for="`perm-${permission.id}`" class="flex-1 cursor-pointer text-sm leading-tight font-normal">
                                                {{ permission.name }}
                                            </Label>
                                        </div>
                                    </div>
                        ...
                        <div class="flex justify-end gap-3 border-t pt-6">
                            <Button type="button" variant="outline" as-child :disabled="processing" v-if="can('roles.canView')">
